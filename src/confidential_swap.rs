...
#[derive(Debug)]
pub enum SwapError {
    ExchangeRateExceedThreshold { actual_rate: u64, expected_rate: u64, max_allowed_diff: u64 },
    ProofCreationError(bulletproofs::ProofError)
}
impl fmt::Display for SwapError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SwapError::ExchangeRateExceedThreshold { actual_rate, expected_rate, max_allowed_diff } => {
...
impl ConfidentialSwap {
    pub fn new(alice_asset: u64,
        bob_asset: u64,
        proof_bits: usize) -> Self {
    ...
    pub fn prove_exchange_rate(&mut self, expected_rate: u64, proof_bits: usize, max_allowed_diff:u64) -> Result<&RangeProof, SwapError> {
        ...
        let commitment = self.pc_gens.commit(
            Scalar::from(rate_diff),
            blinding_factor
        );
